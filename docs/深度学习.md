...
使用2x2 的卷积想要获得较大的感受野，需要更多的网络层次，比如想要获得 5x5 的感受野，可以使用两层3x3的卷积，如果使用2x2 的卷积则需要四层。越深的网络越容易梯度消息，越难训练，使用 2x2 的卷积会在一定程度上增加网络训练的难度。

两层 3x3 的卷积的参数数量为3x3x2=18，一层 5x5 的卷积的参数数量为5x5=25。使用两层3x3的卷积代替一层5x5的卷积参数数量减少了 7。两层2x2 的卷积的参数数量为2x2x2=8，一层3x3 的卷积的参数数量为3x3=9。使用两层2x2 的卷积替代一层3x3 的卷积参数数量只减少了 1。一层2x2 的卷积加上一层3x3 的卷积的参数数量为2x2+3x3=13，一层4x4的卷积的参数数量为4x4=16。使用一层2x2 的卷积加上一层3x3替代一层 4x4 的卷积参数数量减少了 3.
...
没必要：
1. 给神经网络加 L1 正则，然后用大部分深度学习框架自带的优化器（主要是 SGD 及其变种）训练获得不了稀疏解。
2. 实践中如果想获得稀疏解，可以先给目标函数加 L1 正则，然后在训练过程中或者训练结束后把绝对值较小的参数抹成零。
3. 如果稀疏解的模式是无规律的，这种稀疏意义并不大，因为对部署并不友好，完全有可能减了参数却没减推断速度（稀疏的卷积或者其他稀疏运算其实不好实现），模型的存储格式也要重新修改（比如说要存储非零元素的位置），有一连串的后续反应。如果希望稀疏解的模式有规律，往往需要更复杂的正则手段（例如 Group LASSO）来引导，这样一来代码写起来就很麻烦
4. 如果是为了模型压缩、加速推断的话，有搞稀疏正则的时间不如去搞量化、蒸馏以及精简网络结构（MobileNet 等）；如果是为了给模型加正则防止过拟合，不如去用 L2 正则或者 Dropout 等手段，好用又好写。
...
更有原则性的方法来扩大 CNN 的规模，从而可以获得更好的准确性和效率。
用移动反向bottleneck卷积(MBConv) ，类似于 mobileenetv2和 MnasNet

![resnetv2.png](https://github.com/vivienzou1/Deep-Learning-Interview-Book/blob/master/docs/imgs/000EN.png)
![resnetv2.png](https://github.com/vivienzou1/Deep-Learning-Interview-Book/blob/master/docs/imgs/000EN2.png)
**参考资料**
...
## multi-label怎么解决？
* 输出空间呈现指数级增长。那么目前对于Multi-Label的这类特性，学界一般是集中在更好的分析Label之间的相关性，就可以避免这种指数增长。
...
* common solutions:
1. Binary Relevance，这种技巧最简单，它基本上是将每个标签看作一个单独的类分类问题。
![resnetv2.png](https://github.com/vivienzou1/Deep-Learning-Interview-Book/blob/master/docs/imgs/000ML1.png)

* pro and con:
- 这种技巧最简单，它基本上是将每个标签看作一个单独的类分类问题。

2. Classifier Chains，把原问题分解成有先后顺序的一系列Binary Classification，然后前边的Binary Classification会对后边的产生影响；第一个分类器，只用输入数据训练，然后在输入空间以及链条上面所有分类器，训练接下来的每一个分类器。

这和二元关联非常相似，唯一的区别是它是按顺序组成链条以保存标签之间的关联。我们用multi-learn库实现这种方法。

![resnetv2.png](https://github.com/vivienzou1/Deep-Learning-Interview-Book/blob/master/docs/imgs/000ML2.png)

3. Label Powerset： 我们将问题转换为一个多类问题，我们用在训练数据中发现的所有唯一标签的组合训练一个多类分类器

![resnetv2.png](https://github.com/vivienzou1/Deep-Learning-Interview-Book/blob/master/docs/imgs/000ML3.png)

* pro and con:
由于训练数据增加，类的数量相应变得更多。这样以来，会增加模型的复杂性，会降低准确率。
